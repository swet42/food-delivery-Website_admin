...
const Orders = () => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          'https://food-delivery-websitefrontend-verce.vercel.app/api/orders/getall',
          {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.put(`https://food-delivery-websitefrontend-verce.vercel.app/api/orders/getall/${orderId}`, { status: newStatus });
                          ...
                          <div className="space-y-1 max-h-52 overflow-auto">
                            {order.items.map((itm, idx) => (
                              <div key={idx} className="flex items-center gap-3 p-2 rounded-lg">
                                <img src={`https://food-delivery-websitefrontend-verce.vercel.app${itm.item.imageUrl}`} alt={itm.item.name} className="w-10 h-10 object-cover rounded-lg" />
